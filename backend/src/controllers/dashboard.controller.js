...
const clickOnJobGraph = async (recruiterId) => {
  const clickOnJob = await sql.query(`SELECT COUNT(*) as count, DATE(job_click.time) as datetime FROM job_click WHERE recruiter_id = '${recruiterId}' AND time BETWEEN NOW() - INTERVAL 30 DAY AND NOW() GROUP BY datetime`)
  return clickOnJob
}
...
const incompleteCountRecruiter = async (recruiterId) => {
  ...
  for (let index = 0; index < jobIdList.length; index++) {
    const jobId = jobIdList[index]
    const saved = await sql.query(`SELECT COUNT(*) as count FROM incomplete_application WHERE job_id = '${jobId}'`)
    count += saved[0].count
  }
  return count
}
