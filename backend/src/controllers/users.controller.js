...
exports.connections = async (req, res, next) => {
  ...
  try {
    ...
    var connections = []
    var obj = {}
    let result = await neo4jSession.run('MATCH ({userid:$emailadd})--(b) return b', {emailadd: uId})
...
exports.mutual = async (req, res, next) => {
  ...
  try {
    ...
    var suggested = []
    var obj = {}
    let result = await neo4jSession.run('MATCH ( {userid:$emailadd})--(b)--(c) return c', {emailadd: uId})
    console.log('rr', result)
    for (let index = 0; index < result.records.length; index++) {
      const conn = result.records[index]
      let temp = JSON.stringify(conn)
